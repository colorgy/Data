...
class SpreadsheetGenerator
  ...
  def self.generate filename, opts = {}
    ...
    if not @@courses.empty?
      @@courses.map{|cor| cor["department"]}.uniq.each do |department|
        ...
        department = "無系所" if department.nil? or department.empty?
  ...
  private
    def self.select_courses_by opts = {}
...
SpreadsheetGenerator.generate(ARGV[0], output_folder: ARGV[1])
